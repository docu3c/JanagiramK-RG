...
def extract_text_and_metadata(file):
    try:
        if file.type == "application/pdf":
            ...
            metadata_cleaned = {k[1:]: v for k, v in metadata.items() if v}
            pages = {i+1: reader.pages[i].extract_text() for i in range(len(reader.pages))}
            return metadata_cleaned, pages
...
st.markdown("Upload a legal document (.docx or .pdf) to evaluate and improve your legal writing.")
